...
@TeleOp(name = "MasterTeleOp_VectorDrive_VisionOdometry_V3_3", group = "TeleOp")
public class MasterTeleOp_VectorDrive_VisionOdometry_V3_3 extends OpMode {
    // mm - millimeters
    // --- Drivetrain ---
    ...
    // ADJUSTED PIDF for stability (F=12, P=350)
    private final PIDFCoefficients LAUNCHER_PIDF = new PIDFCoefficients(350, 0, 0, 12);

    // NEW: Realistic RPM targets for D-pad control (Max theoretical is 312 RPM)
    private final int RPM_TARGET_UP = 300;     // Max Stable Shot
    private final int RPM_TARGET_RIGHT = 280;
    private final int RPM_TARGET_DOWN = 260;
    private final int RPM_TARGET_LEFT = 240;   // Min Stable Shot
    ...
    private void handleFlywheel(boolean dpadUp, boolean dpadRight, boolean dpadDown, boolean dpadLeft, double manualPower, double voltageComp) {
        ...
        if (manualPower > 0.1) {
            // MANUAL OVERRIDE (Left Trigger is pressed)
            currentTargetRPM = 0;
            launcherMotor.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
            // Apply voltage compensation to manual power to reach max speed.
            double compensatedPower = clamp(manualPower * voltageComp, 0.0, 1.0);
    ...
    // **FIXED:** Corrected formula for 5203 motor (19.1:1)
